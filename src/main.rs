...
fn main() {
    ...
    bg.1.fill_palette(1);
    {
        ...
        bg.1.set_cur_pos(1, 4)
            .put_achar(&AChar::new('C', 4, BgSymmetry::Normal));
        bg.1.set_cur_pos(4, 4)
            .put_achar(&AChar::new(0x80 as u32, 4, BgSymmetry::Normal));
        for i in 0..8 {
            let y = 6 + i as i32 * 2;
            let achar = AChar::new('0' as u32 + i, 4, BgSymmetry::Normal);
            bg.1.set_cur_pos(0, y)
                .put_string("LCR", Some(&CharAttributes::new(5, BgSymmetry::Normal)));
            bg.1.set_cur_pos(4, y)
                .put_achar(&achar)
                .put_achar(&AChar::new(':', 1, BgSymmetry::Normal));
        ...
        bg.1.set_achar_at(26, 4, &AChar::new('+', 4, BgSymmetry::Normal));
        bg.1.set_cur_pos(30, 3).put_string(&"Sound:", None);
        bg.1.set_achar_at(38, 3, &AChar::new('-', 6, BgSymmetry::Normal));
        bg.1.set_achar_at(39, 3, &AChar::new('+', 6, BgSymmetry::Normal));
        bg.1.set_cur_pos(29, 4).put_string(&"Volume:", None);
        bg.1.set_achar_at(38, 4, &AChar::new('-', 7, BgSymmetry::Normal));
        bg.1.set_achar_at(39, 4, &AChar::new('+', 7, BgSymmetry::Normal));
    }
    ...
    'main_loop: loop {
        ...
        let (m_pos_spx, m_pos_spy) = (
            pointer_pos.0 as i32 / PIXEL_SCALE,
            pointer_pos.1 as i32 / PIXEL_SCALE,
        );
        let (m_pos_bgx, m_pos_bgy) = (
            m_pos_spx / PATTERN_SIZE as i32,
            m_pos_spy / PATTERN_SIZE as i32,
        );
        bg.0.set_cur_pos(0, 0).put_string(
            &format!("{:4}", t_count),
            Some(&CharAttributes::new(1, BgSymmetry::Normal)),
        );
        bg.0.set_cur_pos(0, 1).put_string(
            &format!("{:3} {:3}", m_pos_spx, m_pos_spy),
            Some(&CharAttributes::new(1, BgSymmetry::Normal)),
        );
        bg.0.set_cur_pos(0, 2).put_string(
            &format!("{:3} {:3}", m_pos_bgx, m_pos_bgy),
            Some(&CharAttributes::new(1, BgSymmetry::Normal)),
        );
        spr.sp[0].pos(Pos::new(m_pos_spx, m_pos_spy)).visible(true);

        let mut selected:Option<usize> = None;
        let mut speed_ctl:Option<Direction> = None;
        let mut music_select_ctl:Option<Direction> = None;
        let mut gain_ctl:Option<Direction> = None;
        if input_role_state.get(InputRole::LeftButton).1 & 0b1111 == 0b0011
            || input_role_state.get(InputRole::LeftButton).1 & 0xfff_ffff == 0xfff_ffff
                && t_count % 8 == 0
        {
            bg.1.set_cur_pos(m_pos_bgx, m_pos_bgy);
            let achar = bg.1.read_achar();
            if (4..=7).contains(&achar.palette) {
                ...
                if achar.code == '+' as u32 {
                    match achar.palette {
                        4 => speed_ctl = Some(Direction::Up),
                        6 => music_select_ctl = Some(Direction::Up),
                        7 => gain_ctl = Some(Direction::Up),
                        _ => ()
                    }
                }
                if achar.code == '-' as u32 {
                    match achar.palette {
                        4 => speed_ctl = Some(Direction::Down),
                        6 => music_select_ctl = Some(Direction::Down),
                        7 => gain_ctl = Some(Direction::Down),
                        _ => ()
                    }
                }
                if achar.code == 0x7f {
        ...
        if input_role_state.get(InputRole::Left).1 & 0b1111 == 0b0011
            || input_role_state.get(InputRole::Left).1 & 0xfff_ffff == 0xfff_ffff
                && t_count % 4 == 0
        {
            music_select_ctl = Some(Direction::Down);
        }
        if input_role_state.get(InputRole::Right).1 & 0b1111 == 0b0011
            || input_role_state.get(InputRole::Right).1 & 0xfff_ffff == 0xfff_ffff
                && t_count % 4 == 0
        {
            music_select_ctl = Some(Direction::Up);
        }
        if input_role_state.get(InputRole::Up).1 & 0b1111 == 0b0011
            || input_role_state.get(InputRole::Up).1 & 0xfff_ffff == 0xfff_ffff
                && t_count % 8 == 0
        {
            gain_ctl = Some(Direction::Up);
        }
        if input_role_state.get(InputRole::Down).1 & 0b1111 == 0b0011
            || input_role_state.get(InputRole::Down).1 & 0xfff_ffff == 0xfff_ffff
                && t_count % 8 == 0
        {
            gain_ctl = Some(Direction::Down);
        }
        ...
        play_step += match speed_ctl {
            Some(Direction::Up) if play_step > 1 => -1,
            Some(Direction::Down) if play_step < 4 => 1,
            _ => 0,
        };
        music_select += match music_select_ctl {
            Some(Direction::Up) => 1,
            Some(Direction::Down) => 31,
            _ => 0,
        };
        music_select %= 32;
        master_gain += match gain_ctl {
            Some(Direction::Up) if master_gain < 7 => 1,
            Some(Direction::Down) if master_gain > 0 => -1,
            _ => 0,
        };
        ...
        for music_no in 0..0x20 {
            if bg.1.get_code_at(music_no as i32 + 4, 22) == 'R' as u32
                && !sound_manager.play_progress(music_no)
                && sound_manager.play_request[music_no] == 0
            {
                sound_manager.play_request[music_no] = -1;
            }
        ...
        for ch in 0..NUM_OF_AUDIO_CHANNELS {
            ...
            let gain = if sound_generator.mute[ch] || freq <= 30.0 {
                0
            } else {
                g as i32
            };
            ...
            bg.1.set_cur_pos(6, y)
                .put_string(&format!("{:7.2}Hz {:1} {:2} ", freq, w, gain), None);
            bg.1.set_cur_pos(21, y)
                .put_code_n(0x7f as u32, gain)
                .put_code_n(' ', 15 - gain);
        }
        ...
        for music_no in 0..0x20 {
            let c = if sound_manager.play_progress(music_no) {
                '|'
            } else {
                '_'
            };
        ...
        bg.1.set_cur_pos(24, 2).put_string(
            &format!("{:9} {:6}", audio_device.current(), audio_device.remain()),
            Some(&CharAttributes::new(2, BgSymmetry::Normal)),
        );
        bg.1.set_cur_pos(36, 3)
            .put_string(&format!("{:02X}", music_select), None);
        bg.1.set_cur_pos(37, 4)
            .put_string(&format!("{:1}", master_gain), None);
        ...
        bg.1.set_cur_pos(21, 4)
            .put_string(&format!("{:3}%", speed), None);
